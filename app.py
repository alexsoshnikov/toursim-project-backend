...
for i in range(n-1):
    probl += X[0][i+1] * TimeToGo[start-1][i] + X[i+1][0] * TimeToGo[i][start-1]
for i in range(1,n):
    for j in range(1,n):
...
for el in range(len(total)):
    for i in total[el]:
        probl += i
money = p.LpConstraint(probl,sense=-1,name='Денежное ограничение',rhs=moneyLimit)
#Lprob += money
...
for el in X:
    for i in el:
        minSight += i
minVisit = p.LpConstraint(minSight,sense=1,name='Минимальное число посещенных',rhs=sightsLimit)
Lprob += minVisit
...
timeTr = 0
for i in range(n-1):
    timeTr += X[0][i+1] * TimeToGo[start-1][i] + X[i+1][0] * TimeToGo[i][start-1]
for i in range(1,n):
    for j in range(1,n):
        timeTr += TimeToGo[i-1][j-1]* X[i][j]
upTimeTr = p.LpConstraint(timeTr,sense=-1,name='Суммарное время в транспорте',rhs=trTimeLimit) 
Lprob += upTimeTr
...
cons = p.LpConstraint(lpt,sense=-1,name='Ограничение на суммарное время',rhs=allTimeLimit)
Lprob += cons
...
for i in range(0,n):
    for x in range(0,n):
        summa += X[i][x] - Xt[i][x]
    cons = p.LpConstraint(summa,sense=0,name='Вошёл - вышел{}'.format(i),rhs=0)
    summa, summa0 = 0,0
    Lprob += cons
    cons = 0
...
for j in range(1,n):
    for i in range(0,n):
        if i != j:
             ...
             cons = p.LpConstraint(perem,sense=-1,name='Единственность C{}_{} <='.format(j,i),rhs=n+1)
             Lprob += cons
             perem = C[j] - (C[i]) - n*(X[i][j])
             cons = p.LpConstraint(perem,sense=1,name='Единственность C{}_{} >='.format(j,i),rhs=-n+1)
             Lprob += cons
...
for i in range(0,n):
    perem = X[i][i]
    cons = p.LpConstraint(perem,sense=0,name='Диагональ{}_{}'.format(i,i),rhs=0)
    Lprob += cons
